...
// Destructors cannot have contracts (P2900R14 restriction)
struct U {
  int x;
  ~U() pre(x >= 0);  // expected-error {{contracts may not be specified on a destructor}}
};

struct V {
  int x;
  ~V() post(x >= 0);  // expected-error {{contracts may not be specified on a destructor}}
};
