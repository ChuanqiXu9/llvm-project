...
// === 10. More declaration forms ===

namespace api {
inline namespace v1 {
int inline_namespace_fn(const int x) pre(x > 0) post(r: r >= x) {
  return x;
}
}
}
...
struct OverloadSet {
  int choose(const int x) pre(x > 0) post(r: r >= x) {
    return x;
  }

  double choose(const double x) pre(x > 0.0) post(r: r >= x) {
    return x;
  }
};

[[nodiscard]] int nodiscard_contract(const int x) pre(x > 0) post(r: r >= x) {
  return x;
}

int global_value = 1;
int &ref_return_contract() pre(global_value > 0) post(r: r > 0) {
  return global_value;
}
...
void test_all() {
  ...
  api::inline_namespace_fn(1);
  api::v1::inline_namespace_fn(2);
  OverloadSet overloads;
  overloads.choose(1);
  overloads.choose(1.0);
  int nodiscard_value = nodiscard_contract(1);
  (void)nodiscard_value;
  ref_return_contract();
}
...
decltype(auto) trailing_decltype_auto(int &x) pre(x > 0) post(r: r > 0) {
  return (x);
}

auto trailing_auto_no_result(int x) -> auto pre(x > 0) post(true) {
  return x;
}
