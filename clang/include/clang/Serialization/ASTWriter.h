...
public:
  ...
  uint64_t getDeclsHash() const { return DeclsHash; }
...
protected:
  ...
  uint64_t getEmittedDeclsHash() const { return Writer.getDeclsHash(); }
...
class ThinBMIGenerator : public PCHGenerator {
  std::optional<uint64_t> existing_hash_value;
...
public:
  ThinBMIGenerator(const Preprocessor &PP, InMemoryModuleCache &ModuleCache,
                   ...
                   ArrayRef<std::shared_ptr<ModuleFileExtension>> Extensions,
                   std::optional<uint64_t> hash_value,
