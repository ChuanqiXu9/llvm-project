...
void ASTReader::loadDeclUpdateRecords(PendingUpdateRecord &Record) {
  ...
  if (auto I = PendingSpecializationUpdates.find(ID); I != PendingSpecializationUpdates.end()) {
    auto SpecializationUpdates = std::move(I->second);
    PendingSpecializationUpdates.erase(I);

    for (const auto &Update : SpecializationUpdates)
      AddDeclSpecs(D, Update.Data, *Update.Mod);
  }
}
