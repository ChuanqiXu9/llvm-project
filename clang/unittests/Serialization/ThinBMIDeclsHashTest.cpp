...
public:
  ...
  std::string GenerateModuleInterface(StringRef Contents, StringRef AdditionalArgs = StringRef()) {
...
// Test that:
// - the BMI hash won't change if we only touched the body of a non-inline function.
// - the BMI hash will change if we changed the interface of exported non-inline function.
// - the BMI hash will change if we add or delete exported functions.
// - the BMI hash won't change if we changed, add and delete non-exported and non-inline
//   function.
...
// Tests that:
// - The interface shouldn't change if we changed the definition of the non-inline variables.
// - The interface shouldn change if we change the interface (type and name) of the exported
//   non-inline variables.
// - The interface shouldn't change if we change, add or remove non-exported and non-inline
//   variables.
...
export inline int a(int v = 44) {
  ...
  // implementation strategies to change the interface every time for every change
  // in inline functions. 
  EXPECT_FALSE(CompareBMIHash(R"cpp(
export module a;
export int a() {
...
static std::string get_module_map_flag(StringRef ModuleName, StringRef BMIPath) {
...
export module b;
import a;
  )cpp", get_module_map_flag("a", BMIPathA));
